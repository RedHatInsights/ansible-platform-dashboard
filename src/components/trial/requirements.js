...
const Requirements = ({ afterTrial }) => {
  ...
  const installText = afterTrial
    ? trialMessages.installRHETrial
    : trialMessages.reqCardRHELContent;
  const startInstallText = afterTrial
    ? trialMessages.startInstallTrial
                ...
                {intl.formatMessage(installText, {
                  ...
                  button: (chunks) => (
                    <Button
                      isInline
                      variant="link"
                      onClick={() => downloadTrial(RHEL_CHECKSUM)}
                    >
                      {chunks}
                    </Button>
                  ),
            ...
            <div style={{ flexGrow: 1, alignSelf: 'center' }}>
              <TextContent>
                ...
                {intl.formatMessage(startInstallText, {
                  a1: (chunks) => (
                    <Link link="https://console.redhat.com/ansible/automation-hub">
                      {chunks}
                    </Link>
                  ),
                })}
